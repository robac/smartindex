...
class DefaultRenderer implements \Smartindex\Renderer\iIndexRenderer {
    ...
    private function buildList($data, $namespace, &$document, $level) {
        ...
        foreach($data[$namespace][\DefaultIndexer::KEY_DIRS] as $ns)  {
            ...
            if (($this->config->openDepth > $level) || (isset($data[$ns][\DefaultIndexer::KEY_FOLLOW]))) {
        ...
        foreach($data[$namespace][\DefaultIndexer::KEY_PAGES] as $key => $page) {
            $heading = $data[$namespace][\DefaultIndexer::KEY_PAGES_TITLE][$key];
            if ($heading == "")
                $heading = $page;
